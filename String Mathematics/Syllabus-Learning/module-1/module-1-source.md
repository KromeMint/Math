...
| Superscript left | *k* | **Order** | The power each Prime value is raised to: B~n~ = (P~n~)^k^ |
...
| B^k\|f^ | **Base String** | Each Prime value raised to the *k*-th power: B~n~ = (P~n~)^k^ |
| jS^k\|f^ | **Substring** | A child string, built from relationships between adjacent terms of its parent |

**Two ways of writing the same thing.** When a string is being *evaluated* at an index it is often clearer to write the index in parentheses, function-style: B^2^(4) means exactly what B^2|f^~4~ means. **These are the same object in two notations, not two objects** — the parenthesised form is used wherever the subscript would be hard to read.
...
P^f^ = (P~0~, f(P~0~), f(f(P~0~)), f(f(f(P~0~))), … )
...
|---------------|--------------|------------------------------|-----------------------------------------|
| x + 1 | 0 | 0, 1, 2, 3, 4, 5, … | Regular whole-integer |
| x + d | 0 | 0, d, 2d, 3d, 4d, … | Arithmetic, step *d* |
| 2x | 1 | 1, 2, 4, 8, 16, 32, … | Geometric, ratio 2 |
...
| *n* | Prime value P~n~ | Base operation | Base value B~n~ |
|-----|-----------------------------|--------------------------------------|----------------------------|
| 0 | 0 | 0^3^ | 0 |
| 1 | 1 | 1^3^ | 1 |
| 2 | 2 | 2^3^ | 8 |
...
^C^jS^k|f^~n~ = h((j-1)S^k|f^~n~, (j-1)S^k|f^~n+1~)
...
kS^k|f^ = k! (the Root String — a constant string)

(k+1)S^k|f^ = 0 (the Terminator String)

## 8.2 The canonical example — the cubic String Tree

| Level | Notation | Values |
|-----------------------------------------------|------------------------------|-----------------------|
| Base String | B^3\|f^ | 0, 1, 8, 27, 64, 125, … |
| First Substring | 1S^3\|f^ | 1, 7, 19, 37, 61, … |
| Second Substring | 2S^3\|f^ | 6, 12, 18, 24, … |
| **Root String** | 3S^3\|f^ | 6, 6, 6, 6, … |
| **Terminator String** | 4S^3\|f^ | 0, 0, 0, 0, … |
...
Take the Base String B~n~ = n^k^ and compute one comparative difference:

h(B~n~, B~n+1~) = (n+1)^k^ - n^k^

Expand (n+1)^k^ by the binomial theorem:

(n+1)^k^ = n^k^ + kn^k-1^ + C(k, 2)n^k-2^ + … + 1

Subtracting n^k^ removes the leading term entirely:

(n+1)^k^ - n^k^ = kn^k-1^ + (terms of degree k-2 and below)
...
Step 1 dealt with a *pure power*, n^k^. But after one differencing we are no longer looking at a pure power — we are looking at kn^k-1^ **plus debris**: a collection of lower-order terms. Before applying Step 1 again we must know that the debris cannot interfere.

> **Lemma.** If *p* is a polynomial of degree *m* with leading coefficient *c*, then Δ p has degree m - 1 with leading coefficient c · m.

**Why:** differencing is **linear** — the difference of a sum is the sum of the differences — so we may treat the leading term and the debris separately. Step 1 sends the leading term cn^m^ to cmn^m-1^ plus more debris. The existing debris has degree at most m-1, so *its* differences have degree at most m-2. **Debris can only ever fall further behind; it can never climb back up to the leading position.** So the leading coefficient at each stage is determined entirely by the leading coefficient of the stage before, and the lower-order terms never touch it. □
...
1 · n^k^ → k · n^k-1^ → k(k-1) · n^k-2^ → k(k-1)(k-2) · n^k-3^ → …
...
Differencing a constant gives zero, because adjacent terms are equal. That zero string is the Terminator. ∎
...
(E f)(n) = f(n+1), (I f)(n) = f(n)

One differencing is then exactly Δ = E - I, since Δ f(n) = f(n+1) - f(n).
...
Δ^k^ = (E - I)^k^ = Σ[j=0 … k] (-1)^k-j^ C(k, j) E^j^

Since E^j^ shifts by *j* places, reading this off at index 0 gives:

Δ^k^ f(0) = Σ[j=0 … k] (-1)^k-j^ C(k, j) f(j)
...
Root = Δ^k^[n^k^](0) = Σ[j=0 … k] (-1)^k-j^ C(k, j) j^k^
...
Count them by inclusion–exclusion. There are j^k^ ways to place *k* balls into a chosen subset of *j* boxes with no restriction; there are C(k, j) ways to choose which *j* boxes; and the alternating signs remove the arrangements that miss one or more boxes. The result is exactly the sum above.
...
Now the inclusion–exclusion count: start with all 2^2^ = 4 placements; subtract the ones that miss at least one box — there are C(2, 1) = 2 boxes that could be the sole survivor, and 1^2^ = 1 placement each, so subtract 2; add back the ones missing two boxes, of which there are none. Total 4 - 2 + 0 = 2. ✔
...
Both counts describe the same set of arrangements, so they are equal. ∎
...
**But operator algebra used for a different job is a different matter.** The bridge in §9.6.1 — that Δ^k^ = (E-I)^k^ expands by the binomial theorem because *E* and *I* commute — **holds for every string whatsoever.** It says nothing about polynomials, degrees, or leading coefficients, so it does **not** share the premise that makes the other three arguments one argument.
...
> A Base String is *exactly* B~n~ = (P~n~)^k^ — each Prime value raised to a power. **Question 3 below steps outside that definition**, considering a general polynomial such as 3n^2^ + 5n + 7, which **cannot** be written as (P~n~)^k^ for any Prime String and any order.
>
...
It is tempting to record the three findings as three separate knobs — a step *d*, a multiplier *a*, an order *k* — and write R = a · k! · d^k^.
...
What both are really doing is setting a single number: the **leading coefficient of the string, viewed as a polynomial in the index *n***. Stepping by *d* sets it to d^k^. A multiplier out front sets it to *a*. Doing both sets it to a · d^k^. In every case the Root responds only to that one number:
...
| String | Leading coefficient *A* | Predicted R = A · k! | Observed |
|-------------------|--------------------------------|---------------------------|----------------------|
| n^3^ | 1 | 1 · 6 = 6 | 6 ✔ |
| (2n)^3^ = 8n^3^ | 8 | 8 · 6 = 48 | 48 ✔ |
| 3n^2^ + 5n + 7 | 3 | 3 · 2 = 6 | 6 ✔ |
| 3(2n)^2^ = 12n^2^ | 12 | 12 · 2 = 24 | 24 ✔ |
...
4. **The Root value divided by k! is the leading coefficient *A*.** No further assumption is required — this is R = A · k! read backwards.
...
Instead of stepping down the tree one level at a time, the entire descent collapses into a single expression. **This is derived in §9.6.1** — it is not an independent assumption — by expanding Δ^k^ = (E - I)^k^ with the binomial theorem:
...
For k = 3 this gives B~n~ = n(n-1)(n-2):
...
The Root String is 2S^2|f^ = (2,2,2,2,…) and the Terminator String is 3S^2|f^ = (0,0,0,…).
...
**Exercise 3.** Squaring a step-3 Prime String gives (3n)^2^ = 9n^2^, so the leading coefficient is A = 9. Prediction from the Root Law: R = A · k! = 9 · 2! = 18.
...
| **Comparative Split** | A comparative form storing the result in separated components, e.g. [sign|magnitude] |
...
| jS^k\|f^~n~ | The term at index *n* of the *j*-th substring |
...
| [sign|magnitude] | The Comparative Split form |

## 14.3 Key results

| Result | Statement | Status |
|----------------------------------|----------------------------|---------------------------------------|
| **Root Theorem** | kS^k\|f^ = k! for the Standard Power Base, **all k ≥ 0** | **Theorem** — two independent proofs, §9.5 and §9.6; checked at k = 0 … 10 by two methods |
| Terminator | (k+1)S^k\|f^ = 0 | **Theorem** (§9.5) |
| Root Law | R = A · k!, *A* the leading coefficient | **Theorem** (§9.5 + §10) |
...
| Alternating sum | k! = Σ~j~ (-1)^k-j^C(k, j) j^k^ | **Theorem** (§9.6) |
...
*Recorded because of what it says about fixes: Version 9 removed a genuine defect and created a second one in the same act, in a place the first fix had made newly reachable. The bar had been safe for exactly as long as it was inside mathematics.*


**Version 9.** **The mathematics was invisible, and this is the fault that mattered.** Every mathematical symbol in this document — every variable, subscript and formula — was being written into the Word file as an embedded equation object. **Those objects do not display in LibreOffice.** They were present in the file and absent from the page: the notation table listed six positions of a symbol and showed none of them, and sentences ended at a colon with nothing after it.

**All mathematics is now written as ordinary formatted text** — real subscripts and superscripts, Greek letters as characters — which displays in every reader. Verified by rendering the document through LibreOffice itself and reading the result back, rather than by inspecting the file.

**Two errors were introduced by that conversion and caught before publication**, both worth recording because they are the kind a converter makes silently: a fraction bar became a division sign without brackets, which turned **(1 + √5)/2 into 1 + √5/2** — a false value for φ in a document whose subject is φ; and a command was matched by a shorter command sharing its prefix, corrupting text. **Both were found by auditing every fraction against the pre-conversion source.**

**Table widths were rebuilt again on the corrected text**, with each column guaranteed enough room for its longest unbreakable word.
...
1. **§10.2 — the law was over-parameterised.** Version 2 gave R = a · k! · d^k^, presenting the step *d* and the multiplier *a* as independent knobs. **They are not.** Scaling inside the power and stepping the Prime String produce *the same string, term for term* — verified directly. Both merely set the leading coefficient. The law is now R = A · k!, with the three-parameter form shown as a decomposition rather than the statement.
2. **§10 — a definitional error.** Version 2 called 3n^2^ + 5n + 7 a "Base String". **It is not one:** a Base String is exactly (P~n~)^k^, and no Prime String and order produce that polynomial. The differencing machinery works on any polynomial sequence, but such a sequence is not a Base String and this document no longer calls it one. A caution now appears before the section that needs it.
...
4. **§2.1, §2.2 — the order *k* was described loosely** as "an operation of order *k*". It is a power: B~n~ = (P~n~)^k^. The vaguer phrasing is what previously invited the misreading that a general function slot sits behind the notation.
...
2. **§9.3 added** — verification at k = 7, 8, 9, 10 by an independent computational method, chosen specifically because building four more tables would have repeated the same method rather than tested it. Evidence now spans k = 0 … 10 by two routes.
