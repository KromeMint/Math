...
date: "Module 2 of the String Mathematics syllabus — Version 4"
---
...
**Version 4.** Prerequisite: Module 1, §1–§11. See §16 for the revision history.
...
We write a ratio substring as jS^k|f^~÷~ when it must be distinguished, and simply "the ratio lineage" otherwise. **This is the first of the module's two new notations.**
...
> For a geometric Base String B~n~ = cr^n^ with r ≠ 0:
> 1S~÷~ = r (constant), 2S~÷~ = 1
> **The ratio lineage of a geometric string reaches its Root at depth 1, and the Root is the common ratio.**

**Proof.** B~n+1~/B~n~ = cr^n+1^ / cr^n^ = r, for every *n*. One step, and the constant *c* cancels — **the ratio Root cannot see the leading constant**, exactly as the difference Root cannot see lower-order terms. ∎
...
Module 1's standard Prime String begins at P~0~ = 0. So every Standard Power Base String begins at 0^k^ = 0:

B^3|f^ = (0, 1, 8, 27, 64, 125, …)
...
| **"No change"** | 0 | 1 |
| **"How far"** | | b - a | | how far the ratio sits from 1 |
...
^D^jS with g = log → (log B~0~, log B~1~, log B~2~, …)

then difference → (log B~1~ - log B~0~, …) = (log B~1~/B~0~, …)

which is the logarithm of the ratio substring. Undoing the logarithm:

> ### The Bridge
> ratio lineage = exp ∘ (difference lineage) ∘ log
...
**(a) Everything from Module 1 transfers.** A ratio lineage reaches its Root at depth *k* exactly when the *logarithm* of the string is a polynomial of degree *k* in the index. Geometric strings have log B~n~ = n log r — degree 1 — which is why §2.2 terminated at depth 1 and not at some other depth.
...
| geometric with ratio r ≠ 1 | never terminates | terminates at depth 1 |
| neither | see §9 | see §9 |
...
P = (1, 1, 2, 3, 5, 8, 13, 21, …), P~n+1~ = P~n~ + P~n-1~
...
**Why:** Δ F~n~ = F~n+1~ - F~n~ = F~n-1~, directly from the defining rule. **Differencing a Fibonacci string walks backwards along it.** There is no Root at any depth.
...
F~n+1~/F~n~ - φ = ((-1)^n^)/(φ^n^ F~n~)

**Verified by hand at four consecutive steps:**

| *n* | F~n+1~/F~n~ | gap, computed | (-1)^n^ / (φ^n^ F~n~) |
|-----|----------------|-----------------------------------|---------------------------------------------|
| 1 | 1 | -0.6180339887 | -1/(1.6180339887 · 1) = -0.6180339887 ✔ |
| 2 | 2 | +0.3819660113 | +1/(2.6180339887 · 1) = +0.3819660113 ✔ |
| 3 | 1.5 | -0.1180339887 | -1/(4.2360679775 · 2) = -0.1180339887 ✔ |
| 4 | 1.66… | +0.0486326780 | +1/(6.8541019662 · 3) = +0.0486326780 ✔ |
...
- **The magnitude shrinks by roughly a factor of φ^2^ ≈ 2.618 each step**, because φ^n^ F~n~ grows like φ^2n^.
- **It is never zero.** For any finite *n*, φ^n^ F~n~ is finite, so the gap is a nonzero number. **The ratio lineage cannot terminate, and this is the proof rather than the observation.**
...
Take the quantity L(x) = x^2^ - x - 1, which is zero exactly at x = φ, and evaluate it at each ratio:

L(F~n+1~/F~n~) = (F~n+1~^2^ - F~n+1~F~n~ - F~n~^2^)/F~n~^2^ = ((-1)^n^)/F~n~^2^

**by Cassini's identity** F~n+1~F~n-1~ - F~n~^2^ = (-1)^n^.

**Checked exactly, no decimals:**

| *n* | ratio *x* | L(x) = x^2^ - x - 1 | (-1)^n^ / F~n~^2^ |
|------|------------------------|----------------------------------------------|-------------------------|
| 2 | 2/1 | 4 - 2 - 1 = 1 | +1/1 = 1 ✔ |
...
> ### The gap between the two lineages is (-1)^n^ / F~n~^2^ — an exact rational number at every step, with no approximation anywhere.
...
A string that is exactly additive and asymptotically multiplicative must satisfy both closures at once. Write *x* for the ratio it is heading toward: adding gives F~n+1~ = F~n~ + F~n-1~, and dividing through by F~n~ gives

x = 1 + 1/x ⟺ x^2^ - x - 1 = 0 ⟺ x = φ
...
**Exercise 4.** Take the mixed string n · 2^n^ for n = 1 … 5: 2, 8, 24, 64, 160. Difference it, then take ratios. **Does either lineage terminate?** What does that tell you about strings built from a polynomial *times* a geometric?
...
**Exercise 6.** Compute L(x) = x^2^ - x - 1 at x = 13/8 exactly, and check it against (-1)^n^ / F~n~^2^.
...
**Exercise 6.** x = 13/8, which is F~8~/F~7~, so n = 7.

L(13/8) = 169/64 - 13/8 - 1 = (169 - 104 - 64)/64 = 1/64

Hmm — the sign is positive while (-1)^7^ is negative. **Check the indexing:** with F~1~ = 1, F~2~ = 1, F~3~ = 2, F~4~ = 3, F~5~ = 5, F~6~ = 8, F~7~ = 13, the ratio 13/8 is F~7~/F~6~, so n = 6, not 7. Then (-1)^6^/F~6~^2^ = +1/64 ✔.
...
| Symbol | Meaning |
|-------------------------------|---------------------------------------------------------------------|
| jS~÷~ | A ratio substring — comparative substring with h(a,b) = b/a |
| [direction|factor] | The Comparative Split in multiplicative form: growth/decay, and by how much |
...
| **The Bridge** | ratio = exp ∘ difference ∘ log — the two lineages are one theory in two coordinate systems |
...
| The Bridge: ratio =exp ∘ difference ∘ log | **Theorem** (§6.2) |
| Δ(r^n^) = r^n^(r-1); geometrics scale, never reduce | **Theorem** (§7) |
| Non-termination is invisible in finite data | **Theorem** (§8.2–§8.3) |
| Fibonacci closed by neither lineage | **Theorem** (§9) |
| Gap = (-1)^n^/(φ^n^ F~n~) | **Theorem**; checked at n = 1,2,3,4 |
| Gap = (-1)^n^/F~n~^2^ via Cassini | **Theorem**; checked exactly at n = 2,3,4,5 |
...
| **Base String** | each Prime value raised to the *k*-th power | the ordered sequence of **registers** for 0 … N |
