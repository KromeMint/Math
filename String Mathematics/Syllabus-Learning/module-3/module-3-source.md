...
date: "Module 3 of the String Mathematics syllabus — Version 5"
---
...
**Version 5.** Prerequisite: Modules 1 and 2. See §11 for the revision history.
...
No Root. Take ratios instead: 4, 3, 2.6…, 2.5 — heading toward 2, never constant. No Root either.
...
We write this lineage jS~(r)~ and call *r* the **tuning**. **This is the module's one new notation.**

The ordinary difference is what you get with the dial at 1.

## 2.2 What it does to a geometric

h~r~(cr^n^, cr^n+1^) = cr^n+1^ - r · cr^n^ = 0
...
This is the calculation the module turns on. Let B~n~ = p(n)r^n^ with *p* a polynomial:

h~r~(B~n~, B~n+1~) = p(n{+}1)r^n+1^ - rp(n)r^n^ = r^n+1^[p(n{+}1) - p(n)]

The bracket is **the ordinary difference of *p***. So:

h~r~[p(n)r^n^] = r · [Δ p(n)] · r^n^
...
> For B~n~ = p(n)r^n^ with deg p = k:
> (k{+}1)S~(r)~ = 0
> **A polynomial of degree *k* times a geometric of ratio *r* is annihilated by the tuned difference at depth k+1.**

**Proof.** By §2.3 each application reduces deg p by exactly one while leaving the factor r^n^ in place. After k+1 applications the polynomial has been differenced past constancy to zero, and zero times anything is zero. ∎

## 2.5 The worked case, by hand

B~n~ = n · 2^n^, so p(n) = n, degree 1, and r = 2. The theorem predicts termination at depth 2.
...
|------------------|----------------------------------------------------------------------------------|
| 1S~(2)~ | 2-2(0)=2 · 8-2(2)=4 · 24-2(8)=8 · 64-2(24)=16 · 160-2(64)=32 |
| 2S~(2)~ | 4-2(2)=0 · 8-2(4)=0 · 16-2(8)=0 · 32-2(16)=0 |

**Exactly as predicted.** And note 1S~(2)~ = 2, 4, 8, 16, 32 = 2^n+1^, which is r · (Δ p) · r^n^ with Δ p = 1 — the formula in §2.3, confirmed term by term.
...
**The ordinary difference.** Everything in Module 1 is the tuned difference with r = 1, and the polynomials it terminates on are exactly p(n) · 1^n^.

## 3.2 Degree zero — Module 2

Take *p* constant, so B~n~ = cr^n^ is purely geometric. Then h~r~ annihilates it in one step, and
...
φ = (1+√5)/2, ψ = (1-√5)/2, with φ + ψ = 1, φψ = -1

Apply the tuned difference **twice, once at each ratio.** Writing h~ψ~ then h~φ~:

h~φ~(h~ψ~ B)~n~ = [B~n+2~ - ψ B~n+1~] - φ[B~n+1~ - ψ B~n~]
= B~n+2~ - (φ + ψ)B~n+1~ + φψB~n~ = B~n+2~ - B~n+1~ - B~n~

> ### The composition of two tuned differences, at the two golden ratios, **is exactly the integer three-term rule.**
> The irrational numbers cancel completely. **φ and ψ enter the calculation and are gone by the end of it** — which is why Fibonacci is a string of whole numbers despite being governed by two irrationals.
...
B~n~ = n^3^, and index 3 is corrupted from 27 to 37 — an error of ε = +10.
...
Read the deviation rows as multiples of ε = 10:
...
> ### The Damage Wedge
> A single error ε at index *m* appears at depth *j* as exactly j+1 consecutive nonzero entries, with values
> ε · (-1)^i^C(j, j-i)
...
3. **How large it is.** The outermost entries carry binomial coefficient C(j, 0) = 1, so **they are ±ε directly.** Here the wedge ends at -10, giving ε = 10.
...
Keep only the **first entry of every level** — the left edge of the tree. For B~n~ = n^3^ that is

0S~0~ = 0, 1S~0~ = 1, 2S~0~ = 6, 3S~0~ = 6
...
| *n* | 0C(n, 0) + 1C(n, 1) + 6C(n, 2) + 6C(n, 3) | Result | n^3^ |
|-----|-----------------------------------------------------------|----------------------|--------------|
| 3 | 0 + 3 + 6(3) + 6(1) | 27 | 27 ✔ |
| 4 | 0 + 4 + 6(6) + 6(4) | 64 | 64 ✔ |
...
**Exercise 2.** B~n~ = (n+1)3^n^ gives 1, 6, 27, 108, 405. **Predict the termination depth from §2.4 before computing**, then compute 1S~(3)~ and 2S~(3)~ and check.

**Exercise 3.** Explain, in one sentence each, why h~1~ recovers Module 1 and why h~r~ on a constant polynomial recovers Module 2.
...
**Exercise 6.** Why does the composition h~φ~ ∘ h~ψ~ produce a rule with no irrational numbers in it, when both tunings are irrational?
...
**Exercise 3.** h~1~(a,b) = b - a is the ordinary difference, so Module 1 is the whole theory with the dial at 1. With *p* constant the string is purely geometric and b − r·a = 0 says exactly b/a = r, so Module 2's ratio test is the degree-zero case.
...
**Exercise 5.** Five numbers — the left edge 0S~0~, 1S~0~, 2S~0~, 3S~0~, 4S~0~. A tree terminating at depth 4 is a degree-4 polynomial, which has five coefficients, so five is both sufficient (§6.1) and minimal (§6.2).

**Exercise 6.** Because the composition depends on φ and ψ only through their **sum** and their **product**, which are 1 and -1 — both integers. The irrationals appear symmetrically and cancel. *This is why a rule with irrational roots can still generate whole numbers forever.*
...
| Symbol | Meaning |
|--------------------------------|--------------------------------------------------------------------|
| h~r~(a,b) = b − r·a | The **tuned difference**, with tuning *r* |
| jS~(r)~ | The *j*-th substring under the tuned difference at *r* |
...
| Result | Status |
|--------------------------------------------------------|--------------------------------------------|
| h~r~ passes through a geometric and differences the polynomial | **Theorem** (§2.3) |
| Tuned Root Theorem: termination at depth k+1 | **Theorem** (§2.4), checked at n2^n^ and (n{+}1)3^n^ |
| h~1~ is Module 1; degree-zero h~r~ is Module 2 | **Theorem** (§3.1–§3.2) |
| h~r~ is total where the ratio lineage was partial | **Theorem** (§3.3) |
| c-b-a annihilates Fibonacci at depth 1 | **Theorem** (§4.2), checked at five triples |
| h~φ~ ∘ h~ψ~ = the integer three-term rule | **Theorem** (§4.4) |
